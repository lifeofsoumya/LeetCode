...
public:
    ...
    void traverseLeft(Node * root, vector<int> &res){
        if(!root || (!root->left && !root->right)) return; // if left right null then leaf node, so skip
        
        res.push_back(root->data);
        if(root->left) traverseLeft(root->left, res);
        else traverseLeft(root->right, res); // if left doesn't exist right is boundary element
    }
